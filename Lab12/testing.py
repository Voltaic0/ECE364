...
class Morpher:
    ...
    def __init__(self, lImage, lTri, rImage, rTri):
        ...
        try:
            ...
            raise AttributeError("Image input must be a numpy array!")
...
class ColorMorpher(Morpher):
    ...
    def getImageAtAlpha(self, alpha):
        ...
        for tri in target:
            ...
            oldSpots1 = np.dot(tri.leftMat, np.array([curX, curY, onesArr]))
            oldSpots2 = np.dot(tri.rightMat, np.array([curX, curY, onesArr]))
            newLeftY = oldSpots1[1, :]
            newLeftX = oldSpots1[0, :]
            newRightY = oldSpots2[1, :]
            newRightX = oldSpots2[0, :]
            ...
            for index, coordinates in enumerate(pointArray):
                x, y = coordinates
                blendedImage[int(y)][int(x)] = newColor[index]
        return blendedImage
...
if __name__ == "__main__":
    lefty, righty = loadTriangles("points.left.txt", "points.right.txt")
    ...
    imageio.imwrite("Testing.png", new)
